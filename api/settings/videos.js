...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const rows = await sql`SELECT value FROM site_settings WHERE key = 'youtube_videos'`;
  ...
  if (req.method === 'POST') {
    ...
    // Validate and clean videos (max 20)
    const cleaned = videos.slice(0, 20).map(v => ({
    ...
    await sql`
      ...
      VALUES ('youtube_videos', ${JSON.stringify(cleaned)}::jsonb, NOW())
